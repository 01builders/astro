...
use commonware_codec::{EncodeSize, Error, RangeCfg, Read, ReadExt, Write, varint::UInt};
...
impl Block {
    ...
    fn compute_transactions_digest(txs: &[Vec<u8>]) -> Digest {
        todo!()
    }

    pub fn new(parent: Digest, height: u64, timestamp: u64, transactions: Vec<Vec<u8>>) -> Self {
        let digest = Self::compute_digest(
            &parent,
            height,
            timestamp,
            &Self::compute_transactions_digest(transactions.as_slice()), // TODO: impl caching
        );
        Self {
            parent,
            height,
...
impl Read for Block {
    ...
    fn read_cfg(reader: &mut impl Buf, cfg: &Self::Cfg) -> Result<Self, Error> {
        ...
        let rng_config = RangeCfg::from(0..=1024);
        let max_tx_size = RangeCfg::from(0..=1024);
        let transactions = Vec::<Vec<_>>::read_cfg(reader, &(rng_config, (max_tx_size, ())))?;
        ...
        let digest = Self::compute_digest(
            ...
            height,
            timestamp,
            &Self::compute_transactions_digest(transactions.as_slice()),
        );
        Ok(Self {
            parent,
            height,
